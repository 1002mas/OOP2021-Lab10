...
public final class LambdaUtilities {
    ...
    public static <T> List<Optional<T>> optFilter(final List<T> list, final Predicate<T> pre) {
         ...
         */
        List<Optional<T>> res = new ArrayList<>();
        list.forEach(i -> res.add(Optional.ofNullable(i).filter(pre)));
        return res;
    }
    ...
    public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
        ...
        list.forEach(v -> map.merge(op.apply(v), new HashSet<T>(Arrays.asList(v)), (old, newElement) -> {
            old.addAll(newElement);
            return old;
        }));
        return map;
    }
